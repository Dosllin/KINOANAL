...
def add_rating(request, rating): #Добавить фильм в отложенные и обновить это в базе данных
    films_data[request]['rating'].append(rating)  # Добавляем Оценку фильма в отложенные пользователем
    with open(f'films.json', 'w',encoding="UTF-8") as file:  # открываем файл для записи и я обязательно переписывю его целиком
...
def random_films():
    ...
    random_films = random.sample(list(films_data), 10)
